...
class ZScoreTradingStrategy:
    ...
    def calculate_spread(self, asset1, asset2):
        ...
        for i in range(self.window, len(asset1)):
        ...
        self.spread = np.full(len(asset1), np.nan)
        for i, (hr, intercept) in enumerate(zip(hedge_ratios, intercepts)):
            idx = i + self.window
            self.spread[idx] = asset2[idx] - (hr * asset1[idx] + intercept)
    ...
    def calculate_z_scores(self):
        ...
        for i in range(self.window * 2, len(self.spread)):
            ...
            if len(window_spread) > 10:  # Minimum observations
    ...
    def generate_signals(self, asset1, asset2):
        ...
        for i in range(1, len(z_scores)):
            ...
            if current_position == 0:
                if z_scores[i] > self.z_entry:  # Short spread (short asset2, long asset1)
                    current_position = -1
                    signals[i] = -1
                elif z_scores[i] < -self.z_entry:  # Long spread (long asset2, short asset1)
                    current_position = 1
                    signals[i] = 1
            
            # Exit signals
            elif current_position != 0:
                if (abs(z_scores[i]) < self.z_exit or  # Normal exit
...
class TradingBacktest:
    ...
    def calculate_returns(self, signals, asset1_prices, asset2_prices, positions):
        ...
        for i in range(1, n):
            if positions[i] != 0:
                # Position: +1 means long spread (long asset2, short asset1)
                #          -1 means short spread (short asset2, long asset1)
                daily_return = positions[i] * (asset2_returns[i] - asset1_returns[i])
    ...
    def calculate_metrics(self, returns):
        ...
        profit_factor = (np.sum(positive_returns) / abs(np.sum(negative_returns)) 
                        if len(negative_returns) > 0 and np.sum(negative_returns) < 0 else np.inf)
