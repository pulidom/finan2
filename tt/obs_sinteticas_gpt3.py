...
if __name__ == "__main__":
    ...
    X, Y, Z, S = simulate_cointegrated_with_aux(
        sigma_X_t, sigma_Y_t, kappa_t, theta_t,
...
#    T = 5 * 252  # 5 años de datos diarios
#
#    sigma_X_t, sigma_Y_t, kappa_t, theta_t = generate_regime_parameters(
#        T, regime_length=252, seed=42
#    )
#
#    X, Y = simulate_cointegrated_assets(
#        sigma_X_t, sigma_Y_t, kappa_t, theta_t,
#        mu_X=0.0, mu_Y=0.0, rho=0.8
#    )
#
#    # Cálculo del periodo de reversión (1/kappa)
#    T_rev = 1.0 / kappa_t
#    t_axis = np.arange(T)
#
#    # --------------------------
...
#
#    plt.figure(figsize=(12,3))
#    plt.plot(t_axis, sigma_X_t, label='σ_X(t)', lw=1)
#    plt.plot(t_axis, sigma_Y_t, label='σ_Y(t)', lw=1)
#    plt.title('Volatilidades')
#    plt.legend()
#    plt.grid(True)
#
#    plt.figure(figsize=(12,3))
#    plt.plot(t_axis, T_rev, color='darkred', lw=1)
#    plt.title('Periodo de reversión (1/κ)')
#    plt.xlabel('t (días)')
#    plt.ylabel('Días')
#    plt.grid(True)
#
#    plt.figure(figsize=(12,3))
#    plt.plot(t_axis, theta_t, color='black', lw=1)
#    plt.title('Theta(t) por régimen (nivel de equilibrio del spread)')
#    plt.xlabel('t (días)')
#    plt.grid(True)
#
#    
