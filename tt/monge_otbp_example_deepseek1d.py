...
if __name__ == "__main__":
    ...
    #plt.xlabel(r'$z$')
    #plt.ylabel(r'$x$')
    #plt.savefig('uncertaintyFnX.png')
    # Resolver problema de baricentro
    y_final, s = ot_barycenter_solver(x, z, lambda_val=0.1, n_iter=1, lr=0.001)
    
    ## Simular para z_target
    z_target1 = np.array([0.2, 0.3])
    z_target2 = np.array([-0.2, -0.3])
    #z_target1 = np.array([0.2])
    #z_target2 = np.array([-0.2])
    ...
    if l:
        ...
        plt.subplot(131)
        plt.scatter(z, y_final, alpha=0.5)
    #    plt.plot([x.min(), x.max()], [x.min(), x.max()], 'r--')
        plt.xlabel('z')
        plt.ylabel('y baricentro')
    #    plt.title('Relación entre z e y')
